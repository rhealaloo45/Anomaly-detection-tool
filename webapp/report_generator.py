...
class PDFReport(FPDF):
    ...
    def add_anomaly(self, index, log_details, score, explanation=None):
        self.set_font('Arial', 'B', 10)
        self.cell(0, 6, f"Anomaly #{index} (Score/Error: {score:.4f})", 0, 1)
...
def generate_report(model_name, anomalies, filename):
    ...
    for item in anomalies:
        # Check if item has 'log', otherwise it might be the raw row?
        # In model_utils, we will construct the list of dicts.
        pdf.add_anomaly(
            item.get('index', 0), 
            item.get('log', {}), 
